...
--Total Seats
	...
	FROM [Constituency wise results];
...
FROM 
    [Constituency wise results] cr
JOIN 
    [Statewise results] sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
JOIN 
...
FROM [Constituency wise results] cr
JOIN Partywise_results p ON cr.Party_ID = p.Party_ID
JOIN [Statewise results] sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
JOIN states s ON sr.State_ID = s.State_ID
WHERE s.State = 'BIHAR' AND cr.Constituency_Name = '';
...
FROM 
    [Constituency wise details] cd
JOIN 
    [Constituency wise results] cr ON cd.Constituency_ID = cr.Constituency_ID
WHERE 
...
FROM 
    [Constituency wise details] cd
JOIN 
    [Constituency wise details] cr ON cd.Constituency_ID = cr.Constituency_ID
WHERE 
    cd.EVM_Votes = (
        SELECT MAX(cd1.EVM_Votes)
        FROM [Constituency wise details] cd1
...
FROM 
    RankedCandidates rc
JOIN 
    [Constituency wise results] cr ON rc.Constituency_ID = cr.Constituency_ID
GROUP BY 
    cr.Constituency_Name
ORDER BY 
    cr.Constituency_Name;
...
FROM 
    [Constituency wise results] cr
JOIN 
    [Constituency wise details] cd ON cr.Constituency_ID = cd.Constituency_ID
JOIN 
   [Statewise results] sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
JOIN 
    states s ON sr.State_ID = s.State_ID
